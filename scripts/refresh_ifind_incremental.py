...
def main() -> None:
    ...
    base_updated, base_fetch_summary = refresh_frame(provider, base, base_assets, end)
    ...
    expanded_updated, expanded_fetch_summary = refresh_frame(provider, expanded_new, extra_assets, end)
...
def refresh_frame(
    ...
    if start > end or not assets:
        return existing, {"fetched_rows": 0, "symbols": len(assets), "start": None, "end": None}

    fetched = provider.fetch_etf_daily(assets, start, end)
    if fetched.empty:
        return existing, {"fetched_rows": 0, "symbols": len(assets), "start": start.isoformat(), "end": end.isoformat()}
