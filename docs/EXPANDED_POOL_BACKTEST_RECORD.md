...
`2026-07-08` 已使用 iFinD HTTP 历史行情接口增量刷新原始 ETF 池和新增 10 只 ETF 的真实日线行情：

- 数据区间：`2021-01-04` 至 `2026-07-08`
- 新增 ETF 数量：10 只
- 新增日线记录：13,013 行
- 合并后扩展池数量：29 只
...
|---|---|---:|---:|---:|---:|---:|---:|---:|
| 原19只ETF池 | Top3 25%上限+现金 | 82.67% | 13.29% | 19.73% | 0.67 | -17.02% | 1.08 | 6.47% |
| 扩展A股ETF池 | Top3 25%上限+现金 | 128.24% | 16.87% | 21.35% | 0.79 | -18.53% | 1.16 | 7.22% |
| 原19只ETF池 | Top3满仓 | 114.21% | 17.09% | 26.31% | 0.65 | -22.49% | 1.10 | 8.63% |
| 扩展A股ETF池 | Top3满仓 | 184.90% | 21.87% | 28.47% | 0.77 | -24.63% | 1.16 | 9.62% |
| 原19只ETF池 | Top4满仓 | 82.23% | 13.23% | 25.01% | 0.53 | -22.93% | 0.99 | 7.27% |
| 扩展A股ETF池 | Top4满仓 | 115.44% | 15.60% | 26.23% | 0.59 | -23.50% | 1.01 | 8.88% |
...
扩展池确实提高了收益弹性：主分支 `Top3 25%上限+现金` 的总收益从 `82.67%` 提高到 `128.24%`，年化收益从 `13.29%` 提高到 `16.87%`。

代价也明确：最大回撤从 `-17.02%` 扩大到 `-18.53%`，年化波动从 `19.73%` 提高到 `21.35%`，平均日换手从 `6.47%` 提高到 `7.22%`。信息比率从 `1.08` 提高到 `1.16`，说明扩池后的收益提升在本次刷新后不仅来自成长/科技弹性，也提升了一部分超额效率。
...
|---|---:|---:|---:|---:|
| ML overlay，阈值0.50 | 19.71% | 1.24 | -16.51% | 12.44% |
...
|---:|---:|---:|---:|---:|
| 0.50 | 19.71% | 1.24 | -16.51% | 12.44% |
| 0.55 | 17.67% | 1.11 | -16.68% | 6.91% |
| 0.60 | 17.62% | 1.11 | -16.68% | 4.74% |
| 0.65 | 17.48% | 1.09 | -16.68% | 3.83% |
...
- 相比固定 75% 仓位，ML overlay 的平均收益从 `18.91%` 提高到 `19.71%`，平均 Sharpe 从 `1.16` 提高到 `1.24`。
- 本次刷新后 ML overlay 的最差回撤与固定 75% 仓位基本持平，说明它在扩展池上更像“有限进攻 + 近似同等回撤”的增强分支。
