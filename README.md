...
截至当前已接入的真实 iFinD ETF 数据，最新数据日期为 `2026-07-08`，最近一次组合调仓日为 `2026-07-03`：
...
数据说明：ETF 历史日线已通过 iFinD HTTP 回填并增量刷新至 `2026-07-08`，当前本地原始行情最早可用日期为 `2019-01-02`。由于 `2026-07-08` 是周三，不是周五调仓日，因此最新推荐组合仍使用 `2026-07-03` 的周度信号。刷新过程中已对 ETF 拆分/复权口径导致的交易价格跳变做连续性修正，避免把技术性除权误计为真实回撤。
...
- 存储方式：本地 Parquet 文件
- 主策略回测区间：`2021-01-01` 至 `2026-07-08`
- 长样本研究区间：`2019-01-01` 至 `2026-07-08`
...
- `docs/LONG_SAMPLE_2019_BACKTEST_RECORD.md`
- `docs/SINGLE_YEAR_PERFORMANCE_RECORD.md`
